...
const practiceable = () => skills.filter(skill => skill.practiceable && canAccessSkill(state, skill.id));
...
function levelStatus(level) { if(!canAccessLevel(state,level))return "🔒 Locked";return isLevelCompleted(state,level)?"✅ Completed":"▶ Available"; }
...
function dashboard() {
  const courseSkill=nextLesson(state),rec=recommendedSkill(state),practiceSkill=rec.skill,score=proficiency(practiceSkill.id),course=lessonStats(state);
  ...
  const practiceMessage = errorAdvice || (rec.days >= 7 && score ? `你已经 ${Math.floor(rec.days)} 天没有练习 ${practiceSkill.title}，建议复习一下。` : score >= 80 ? `你对 ${practiceSkill.title} 已经比较熟悉，可以用变化棋形保持手感。` : score ? `继续练习 ${practiceSkill.title}，把理解变成直觉。` : `可以从 ${practiceSkill.title} 开始建立熟练度。`);
  const courseMessage=courseSkill?`下一课：${courseSkill.title}。课程进度只看是否完成教学，不受熟练度高低影响。`:`当前已开放课程全部完成。你可以复习、练习，或查看后续学习路径。`;
  const todaySeconds=state.dailyStudySeconds?.[today()]||0;
  return `<main class="page dashboard"><section class="hero"><div><p class="eyebrow">YOUR LONG-TERM GO JOURNEY</p><h1>围棋会一直陪你进步。</h1><p>${courseMessage}</p><div class="hero-actions">${courseSkill?`<button class="button primary" data-action="learn-skill" data-skill="${courseSkill.id}">继续学习 · ${courseSkill.title} <span>→</span></button>`:`<button class="button primary" data-view="path">查看后续学习路径 <span>→</span></button>`}<button class="button ghost" data-action="quick-practice" data-skill="${practiceSkill.id}">Quick Practice</button></div></div><div class="progress-orbit"><div><strong>L${state.profile.level}</strong><span>Beginner</span></div></div></section><section class="stats-grid">${stat(`Level ${state.profile.level}`,"当前课程阶段",`${course.completed}/${course.total} 课已完成`)}${stat(minutes(todaySeconds),"今日学习时间",`累计 ${minutes(state.studyTimeSeconds)}`)}${stat(totalQuestions(state),"已完成题目",`正确率 ${totalAccuracy(state)}%`)}${stat(`${state.streak} 次`,"连续学习",`最长 ${state.longestStreak} 次`)}</section><section class="recommend-card dashboard-goals"><div class="course-goal"><p class="eyebrow">CONTINUE LEARNING · 继续学习</p><h2>${courseSkill?.title||"已完成当前课程"}</h2><p>${courseSkill?.description||"课程完成状态已保存，之后新增 Level 时可继续前进。"}</p><small>Course Progress · 只由 lessonProgress 决定</small>${courseSkill?`<button class="button primary" data-action="learn-skill" data-skill="${courseSkill.id}">进入下一课 →</button>`:`<button class="button ghost" data-view="path">查看学习路径 →</button>`}</div><div class="practice-goal"><p class="eyebrow">RECOMMENDED PRACTICE · 推荐练习</p><h2>${practiceSkill.title}</h2><p>${practiceMessage}</p>${masteryBar(score)}<small title="熟练度用于练习推荐和长期复习，不影响课程解锁。">Mastery ${score}% · ${masteryLabel(score)} · 不影响课程解锁 ⓘ</small><div class="recommend-actions"><button class="button ghost" data-action="quick-practice" data-skill="${practiceSkill.id}">随机练习</button><button class="button ghost" data-action="start-adaptive">Adaptive Training</button></div></div></section><section class="section-heading"><div><p class="eyebrow">AVAILABLE LESSONS</p><h2>课程与熟练度分别显示</h2></div><button class="text-button" data-view="path">全部路径 →</button></section><section class="dash-skills">${practiceable().slice(0,6).map(skillCard).join("")}</section></main>`;
}

function skillCard(skill) { const score=proficiency(skill.id),complete=isLessonCompleted(state,skill.id);return `<article class="skill-card"><div><span class="skill-level">LEVEL ${skill.level}</span><h3>${skill.title}</h3><p>${skill.description}</p><small class="course-state">课程：${complete?"✅ Completed":"▶ Available"}</small></div>${masteryBar(score)}<footer title="Mastery 只影响练习推荐，不影响课程解锁。"><span>熟练度 · ${masteryLabel(score)}</span><b>${score}%</b></footer><button data-action="learn-skill" data-skill="${skill.id}">${complete?"复习课程":"继续学习"} →</button></article>`; }
function path() {
  return `<main class="page"><section class="page-title"><p class="eyebrow">LEARNING PATH</p><h1>学习路径</h1><p>Course Progress 决定解锁；Mastery 只用于练习与复习推荐。两者互不替代。</p></section><section class="level-path">${levels.map(level=>{const access=levelAccess(state,level.id),levelSkills=skills.filter(skill=>skill.level===level.id);return `<article class="level-node ${access} ${isLevelCompleted(state,level.id)?"completed":""}"><header><span>LEVEL ${level.id}</span><b class="node-status">${levelStatus(level.id)}</b></header><h2>${level.title}</h2><p>${level.description}</p><div class="path-skills">${levelSkills.map(skill=>{const score=proficiency(skill.id),complete=isLessonCompleted(state,skill.id),available=canAccessSkill(state,skill.id),courseStatus=complete?"✅ Completed":available?"▶ Available":"🔒 Locked";return `<button ${available?"":"disabled"} class="path-skill ${complete?"lesson-completed":""}" data-action="learn-skill" data-skill="${skill.id}"><i>${complete?"✓":available?"▶":"⌁"}</i><span>${skill.title}<small class="course-line">课程 Course：${skill.practiceable?courseStatus:available?"Planned · 后续开放":"🔒 Locked"}</small>${skill.practiceable?`${masteryBar(score)}<small class="mastery-line" title="Mastery 只影响推荐练习，不影响任何课程或 Level 解锁。">熟练度 Mastery：${score}% · ${masteryLabel(score)} ⓘ</small>`:""}</span></button>`}).join("")}</div></article>`}).join("")}</section></main>`;
}
...
function tutorialCopy(skill) {
  ...
  const lessonDone=isLessonCompleted(state,skill.id),plan=guidedPracticePlans[skill.id]||{count:1,summary:"正确完成一次关键操作。"};
  return `<section class="tutorial-copy"><article class="learning-card knowledge"><div class="card-label"><span>01</span>新知识</div><h2>${title}</h2><p>${t.opening}</p></article>${lessonVisual(skill)}<section class="learn-points">${t.points.map((point,i)=>`<article><span>0${i+1}</span><p>${point}</p></article>`).join("")}</section>${skill.id==="intro" ? introQuiz(t) : `<section class="practice-cta"><div><p class="eyebrow">${lessonDone?"LESSON COMPLETED · OPTIONAL REVIEW":"MINIMUM USEFUL PRACTICE"}</p><h2>${lessonDone?"课程已完成：复习或继续练习都由你选择":plan.count===1?"正确完成一次就进入下一节":`完成 ${plan.count} 个不同练习`}</h2><p>${lessonDone?"重新查看讲解不会改变课程状态；随机练习只会更新 Mastery。":`${plan.summary} 相同操作不会为了凑题数而重复。`}</p></div><div class="practice-cta-actions">${lessonDone?`<button class="button ghost" data-action="review-lesson">重新查看讲解</button>`:""}<button class="button primary" data-action="${lessonDone?"quick-practice":"start-lesson-session"}" data-skill="${skill.id}">${lessonDone?"继续随机练习":plan.count===1?"开始关键操作":`开始 ${plan.count} 个练习`} →</button></div></section>`}</section>`;
}
...
function learn() { const skill=skillById[selectedSkill]||skillById.intro,unlocked=canAccessSkill(state,skill.id),score=proficiency(skill.id),learned=isLessonCompleted(state,skill.id),courseStatus=learned?"LESSON COMPLETED ✓":unlocked?"AVAILABLE":"LOCKED";return `<main class="page learn-page"><section class="learn-heading"><div><p class="eyebrow">LEVEL ${skill.level} · ${courseStatus}</p><h1>${skill.title}</h1><p>${skill.description}</p></div><div class="mastery-summary" title="熟练度用于推荐练习和长期复习，不影响课程、Skill 或 Level 解锁。"><strong>${score}%</strong>${masteryBar(score)}<span>Mastery · ${masteryLabel(score)} · 不影响解锁 ⓘ</span></div></section><section class="skill-picker">${skills.filter(s=>canAccessSkill(state,s.id)).map(s=>`<button class="${selectedSkill===s.id?"active":""}" data-action="learn-skill" data-skill="${s.id}">${s.title}<small>${isLessonCompleted(state,s.id)?"课程完成 · ":"课程可学 · "}Mastery ${proficiency(s.id)}%</small></button>`).join("")}</section>${unlocked?tutorialCopy(skill):`<section class="future-card"><h2>先完成前一课的教学与必要练习</h2><p>这里只检查 Course Progress。Mastery 无论是 1% 还是 100%，都不会改变课程解锁状态。</p></section>`}</main>`; }
...
function nextLearningSkill() { return nextLesson(state); }
...
function progress() { const last=state.trainingSessions.at(-1),todaySeconds=state.dailyStudySeconds?.[today()]||0,course=lessonStats(state);return `<main class="page"><section class="page-title"><p class="eyebrow">PROGRESS</p><h1>你的长期进度</h1><p>Course Progress 表示课程是否完成；Mastery 表示长期练习熟练度。两套数据独立保存。</p></section>${sessionReport(last)}<section class="stats-grid big">${stat(totalQuestions(state),"总练习次数",`正确 ${totalCorrect(state)} 次`)}${stat(`${totalAccuracy(state)}%`,"正确率",`最近练习会影响推荐`)}${stat(minutes(state.studyTimeSeconds),"总学习时间",`今天 ${minutes(todaySeconds)}`)}${stat(`${state.streak} 次`,"当前连续学习",`最长 ${state.longestStreak} 次`)}</section><section class="course-progress-summary"><div class="section-heading"><div><p class="eyebrow">COURSE PROGRESS</p><h2>课程进度</h2></div><span>只由 lessonProgress 决定</span></div><div class="course-progress-grid">${stat(course.completed,"✅ 已完成 Completed",`${course.total} 个已实现课程`)}${stat(course.available,"▶ 可学习 Available","当前可继续的课程")}${stat(course.notStarted,"🔒 未开始 Not Started","等待前置课程完成")}</div></section><section class="topic-stats"><div class="section-heading"><div><p class="eyebrow">SKILL MASTERY</p><h2>每个知识点的熟练度</h2></div><span title="Mastery 只影响练习推荐，不影响课程解锁。">练习推荐数据 · 不影响解锁 ⓘ</span></div>${skills.filter(s=>s.level<=2).map(s=>`<div class="topic-row"><b>${s.title}</b><div class="topic-track"><i style="width:${proficiency(s.id)}%"></i></div><span>${proficiency(s.id)}%</span><small><b class="inline-course-state">课程：${isLessonCompleted(state,s.id)?"✅ Completed":canAccessSkill(state,s.id)?"▶ Available":"🔒 Not Started"}</b> · Mastery：${masteryLabel(proficiency(s.id))} · ${skillAccuracy(state,s.id)}% 正确</small></div>`).join("")}</section></main>`; }
...
app.addEventListener("click",event=>{
  ...
  if(action==="learn-skill"){if(!canAccessSkill(state,target.dataset.skill))return;selectedSkill=target.dataset.skill;state.profile.currentSkill=selectedSkill;save();view="learn";render();return;}
  if(action==="review-lesson"){document.querySelector(".knowledge")?.scrollIntoView?.({behavior:"smooth",block:"start"});return;}
  if(action==="start-lesson-session"){if(!canAccessSkill(state,target.dataset.skill))return;return startLessonSession(target.dataset.skill);}
