...
function practice() {
  ...
  return `<main class="page practice-page"><section class="practice-top"><div><p class="eyebrow">${lessonSession?"GUIDED SKILL PRACTICE":"INFINITE PRACTICE"}</p><h1>${lessonSession?`${skillById[q.skill].title} · ${plan.count} 个关键练习`:"随机练习"}</h1><p>${lessonSession?`${plan.summary} 不重复相同操作。`:"棋形会改变，答案由规则引擎计算。你可以随时选择知识点继续巩固。"}</p></div>${lessonSession?"":`<div class="topic-picker">${available.map(skill=>`<button class="${q.skill===skill.id?"active":""}" data-action="new-topic" data-skill="${skill.id}">${skill.title}</button>`).join("")}</div>`}</section>${sessionPanel()}<section class="practice-question">${questionCard(q)}</section></main>`;
}
...
function makeQuestion(skillId) {
  ...
  let generated=null;
  for(let attempt=0;attempt<24;attempt++){generated=generateQuestion(skillId,proficiency(skillId));if(!seen.includes(generated.signature))break;}
  ...
  currentQuestionResolved=false;currentQuestionCounted=false;selectedPoints=new Set();practiceBoard=null;
}
...
function grade(q, answer) {
  ...
  if(state.activeSession){
    if(lessonMode){state.activeSession.attempts=(state.activeSession.attempts||0)+1;if(correct&&isCurrent&&!currentQuestionCounted){state.activeSession.completed++;state.activeSession.correct++;state.activeSession.skills[q.skill]=(state.activeSession.skills[q.skill]||0)+1;currentQuestionCounted=true;}}
    else if(firstAttempt){state.activeSession.completed++;state.activeSession.attempts=(state.activeSession.attempts||0)+1;if(correct)state.activeSession.correct++;state.activeSession.skills[q.skill]=(state.activeSession.skills[q.skill]||0)+1;currentQuestionCounted=true;}
  }
...
function startLessonSession(skillId) {
  const plan=guidedPracticePlans[skillId]||{count:1},masteryBefore=proficiency(skillId),masteryPerCorrect=Math.ceil(Math.max(0,40-masteryBefore)/plan.count);
  state.activeSession={type:"lesson",skill:skillId,count:plan.count,completed:0,correct:0,attempts:0,skills:{},seenSignatures:[],masteryBefore,masteryPerCorrect,startedAt:new Date().toISOString()};
...
function startTrainingSession(count) {
  ...
  state.activeSession={type:"training",skill:skillId,count:count==="infinite"?Infinity:Number(count),completed:0,correct:0,attempts:0,skills:{},startedAt:new Date().toISOString()};
  save();makeQuestion(skillId);view="practice";render();
}
function endSession() {
  const s=state.activeSession;if(!s)return;
  const record={...s,endedAt:new Date().toISOString(),accuracy:(s.attempts||s.completed)?Math.round(s.correct/(s.attempts||s.completed)*100):0,masteryAfter:s.skill?proficiency(s.skill):null};
